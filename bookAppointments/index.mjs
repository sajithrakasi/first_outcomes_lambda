...
import { bookAppointment , validateRequestBody} from './bookAppointment.mjs';
...
export async function handler(event) {
 ...
 const tokenCheck = await verifyToken(token);
  ...
  try {
    ...
    const bookedResponse = await bookAppointment(input);
    await redisClient.quit();
    if (bookedResponse.count === 0) {
      return {
        statusCode: 404,
        body: JSON.stringify({
          code: '200',
          status: 'success',
          message: bookedResponse.message || 'No patient data found',
          count: 0,
          data: []
        })
      };
    ...
    return {
      ...
      body: JSON.stringify({
        code: '200',
        status: 'success',
        message: bookedResponse.message || 'Appointment booked successfully',
        data: bookedResponse.data
      })
    };
  } catch (error) {
    await redisClient.quit();
    ...
    return {
      ...
      body: JSON.stringify({
        code: '500',
        status: 'error',
        message: error,//'Handler failed during patient lookup.',
