...
export async function fetchAppointments(appointmentSearch) {
  try {
    const { customer_id, patient_id, emr_id, limit, offset, start_date, end_date, appointment_id } = appointmentSearch;

    const today = moment().startOf('day').format('YYYY-MM-DD');

    const appointmentsQuery = db(`${DB_NAME}.emr_appointment`)
      .select([
    ...
    // Optional filters
    if (appointment_id) {
      appointmentsQuery.andWhere('appointment_id', appointment_id);
    }


    if (start_date && end_date) {
      const startDate = moment(start_date).startOf('day').format('YYYY-MM-DD');
      const endDate = moment(end_date).endOf('day').format('YYYY-MM-DD');
      appointmentsQuery.andWhereBetween('appointment_date', [startDate, endDate]);
    }
    ...
    let filteredPastAppt=[...appointment_data]

    // Pagination logic
    const limit_assign = (limit || 200) - 1;
    const offset_assign = (offset || 1) - 1;
    const paginatedAppointments = filteredPastAppt.slice(offset_assign, offset_assign + limit_assign );

    return {
      message: filteredPastAppt.length === 1
        ? 'Matched 1 Appointment.'
        : `Matched ${filteredPastAppt.length} Appointments. Limit: ${limit_assign + 1}, Offset: ${offset_assign + 1}`,
