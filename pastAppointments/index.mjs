...
import { validateRequestBody, fetchAppointments } from './pastAppt.mjs';
...
export async function handler(event) {
  console.log(`Firstoutcome FetchPastAppointments lambda started in "${environment}"`);
  ...
  try {
    ...
    const PastAppointmentResponse = await fetchAppointments(pastAppointmentSearch);
    console.log('PastAppointmentResponse', PastAppointmentResponse);

    if (PastAppointmentResponse.count === 0) {
      return jsonResponse(200, {
        code: '200',
        status: 'success',
        message: PastAppointmentResponse.message || 'No past appointment data found',
        count: 0,
        data: []
      });
    }
    ...
    return jsonResponse(200, {
      code: '200',
      status: 'success',
      message: PastAppointmentResponse.message || 'Past appointment data retrieved successfully.',
      count: PastAppointmentResponse.count,
      data: PastAppointmentResponse.data
    });
    ...
    return jsonResponse(500, {
      code: '500',
      status: 'error',
      message: 'Handler failed during Fetch past appointment lambda.',
