...
import { validateRequestBody, getProviderTypes } from './fetchProvider.mjs';
...
export async function handler(event) {
  console.log(`Firstoutcome FetchProviders lambda started in "${environment}"`);
  ...
  try {
    ...
    const providerResponse = await getProviderTypes(providerSearch);
    ...
    if (providerResponse.count === 0) {
      return jsonResponse(200, {
        code: '200',
        status: 'success',
        message: providerResponse.message || 'No providers data found',
        count: 0,
        data: []
      });
    }
    ...
    return jsonResponse(200, {
      code: '200',
      status: 'success',
      message: providerResponse.message || 'Providers data retrieved successfully.',
    ...
    return jsonResponse(500, {
      code: '500',
      status: 'error',
      message: 'Handler failed during providers lambda.',
