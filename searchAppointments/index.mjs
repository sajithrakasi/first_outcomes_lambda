...
import { validateRequestBody, fetchAppointments } from './appointmentSearch.mjs';
...
export async function handler(event) {
  console.log(`Firstoutcome FetchAppointments lambda started in "${environment}"`);
  ...
  try {
    ...
    const appointmentResponse = await fetchAppointments(appointmentSearch);
    ...
    if (appointmentResponse.count === 0) {
      return jsonResponse(200, {
        code: '200',
        status: 'success',
        message: appointmentResponse.message || 'No appointment data found',
        count: 0,
        data: []
      });
    }
    ...
    return jsonResponse(200, {
      code: '200',
      status: 'success',
      message: appointmentResponse.message || 'Appointment data retrieved successfully.',
      count: appointmentResponse.count,
      data: appointmentResponse.appointmentDetails
    });
    ...
    return jsonResponse(500, {
      code: '500',
      status: 'error',
      message: 'Handler failed during Fetch appointment lambda.',
